...
def test_known_good_pobox_addresses():
    ...
    for pobox in poboxs:
        assert pext.return_max_address4(pext.seq, pobox).upper() == pobox.upper(), "{} should be a pobox address".format(pobox)

def test_known_good_full_clean_addresses():
    base_addresses = _load_column('data/known_good/full_clean_addresses.csv', 'ADDRESSES')

    for base_address in base_addresses:
        result = pext.return_max_address4(pext.seq, base_address)
        assert isinstance(result, str), "{} should be a string".format(base_address)
        assert result.upper() == base_address.upper(), '{},"{}"'.format(base_address.upper(), pext.encode_from_word_list(pext.w(base_address.lower())))
...
def test_eighty_k_good_street_and_po_samples():
    ...
    for base_address in base_addresses:
        assert pext.return_max_address4(pext.seq, base_address).upper() == base_address.upper(), '{},"{}"'.format(base_address.upper(), pext.encode_from_word_list(pext.w(base_address.lower())))

def test_more_poboxs():
    base_addresses = _load_column('data/known_good/more_poboxs.csv', 'ADDRESSES')

...
def test_base_addresses():
    ...
    for base_address in base_addresses:
        assert pext.return_max_address2(pext.seq, base_address) == base_address.upper(), "{} should be a base address".format(base_address)
...
def test_known_good_pobox_addresses():
    ...
    for pobox in poboxs:
        assert pext.return_max_address2(pext.seq, pobox) == pobox.upper(), "{} should be a pobox address".format(pobox)

def test_known_good_full_clean_addresses():
    base_addresses = _load_column('data/known_good/full_clean_addresses.csv', 'ADDRESSES')

    for base_address in base_addresses:
        assert pext.return_max_address2(pext.seq, base_address) == base_address.upper(), '{},"{}"'.format(base_address.upper(), pext.encode_from_word_list(pext.w(base_address.lower())))
